...
const initialAppList = appsList.filter(eachApp => eachApp.category === 'SOCIAL')

class AppStore extends Component {
  state = {
    search: '',
    activeTabFilterredApps: initialAppList,
    currentActiveTab: 'SOCIAL',
  }
  ...
  activeTabApps = tabId => {
    this.setState({
      activeTabFilterredApps: appsList.filter(
        eachApp => eachApp.category === tabId,
      ),
    })
    this.setState({currentActiveTab: tabId})
  }

  render() {
    const {search, activeTabFilterredApps, currentActiveTab} = this.state
    const filterredApps = activeTabFilterredApps.filter(eachApp =>
      eachApp.appName.toLowerCase().includes(search.toLowerCase()),
    )
    console.log(search)
    return (
